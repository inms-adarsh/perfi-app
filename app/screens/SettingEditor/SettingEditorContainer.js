...
const enhance = compose(
  firestoreConnect(),
  connect((state, props) => {
    const setting = state.firestore.ordered.currentSettings && state.firestore.ordered.currentSettings[0];
    return ({
      setting,
  ...
  withHandlers({
    ...
    onSubmit: ({
      ...
      Keyboard.dismiss();
      const editedProps = R.pick(['name', 'phone', 'address', 'pan', 'email', 'gst', 'notes'/*-- ADD PROPS --*/], props);
