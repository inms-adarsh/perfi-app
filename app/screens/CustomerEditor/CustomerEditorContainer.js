import {
  ...
  import CustomerEditor from './CustomerEditorScreenView';
  import { getParam } from '../../utils/navHelpers';
  import { firestoreConnect } from 'react-redux-firebase'
  /*-- IMPORT SCREENS --*/
  
  const screenProp = (propName, def) => R.pathOr(def, ['customer', propName]); 
  const requiredProps = ['name', 'phone', 'gst', 'address',/*-- ADD PROPS --*/];
  ...
  const enhance = compose(
    ...
    withProps(({ navigation }) => ({ customer: getParam('customer')(navigation) })),
    ...
    withHandlers({
      ...
      onSubmit: ({
        navigation, firestore, auth, profile, listUrl, customer, onClose, ...props
      }) => () => {
        Keyboard.dismiss();
        const editedProps = R.pick(['name', 'phone', 'email', 'gst', 'address',/*-- ADD PROPS --*/], props);
        const propsToSubmit = customer ? Object.assign(customer, editedProps) : editedProps;
        let promise = {};
        if( customer) {
          ...
          promise = firestore.set('tenants/'+ profile.tenantId +'/customers/' + customer.id, propsToSubmit)
        } else {
          Object.assign(propsToSubmit, {
          ...
          promise = firestore.add('tenants/' + profile.tenantId + '/customers', propsToSubmit)
        }

        promise.then(() => {
          ToastAndroid.show('Customer has been saved successfully!', ToastAndroid.BOTTOM);
        }, (e) => {
          ToastAndroid.show('Error saving customer! Please try again', ToastAndroid.BOTTOM);
        });
        if(listUrl) {
    ...
    lifecycle({
      componentDidMount() {
        ...
        if(customer) {
          const { name, /*-- FETCH PROPS --*/ } = customer;
          setName(name);
          /*-- SET PROPS --*/
        }
      },
  ...
  export default hoistStatics(enhance)(CustomerEditor);
